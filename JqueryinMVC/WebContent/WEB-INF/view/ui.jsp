...
<body>
	...
	<div id="dailogMsg" title="Student Information">

		Student First name: ${student.firstName}<br> Student Last name:
		${student.lastName}<br> Gender: ${student.gender}<br>
		Country: ${student.country}<br> Hobbies:
		<ul>
			<c:forEach var="meal" items="${reservation.language}">
				<li>${meal}</li>
			</c:forEach>
		</ul>
		
		<p>Dear <i>${student.firstName},</i> this is the simple Jqeuery UI example</p>
	</div>
		...
		<div style="margin-left: 21%; position: relative;" id="accordion-2">
			...
			<div>
				<p>Struts is an open source framework that extends the Java
					...
					architecture. It enables you to create maintainable, extensible,
					and flexible web applications based on standard technologies, such
					as JSP pages, JavaBeans, resource bundles, and XML</p>
			</div>
		</div>
		...
		<div style="margin-left: 21%; position: relative;" id="tabs-1">
			...
			<div id="tabs-2">
				<p>In Spring MVC, the model works a container that contains the
					...
					objects, strings, information from the database, etc. ... The
					object of HttpServletRequest reads the information provided by the
					user and pass it to the Model interface.</p>
			</div>
			<div id="tabs-3">
				<p>ModelAndView is a holder for both Model and View in the web
					...
					holds both to make it possible for a controller to return both
